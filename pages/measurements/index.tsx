...
const MeasurementsPage: NextPage = (props: any) => {
          ...
          {user.data ? (
            <Grid container direction="column">
              <Grid item xs={12}>
                ...
                >
                  <Tab label="BODY PROFILE" />
                  <Tab label="UPLOAD PICTURES" />
                  <Tab label="MEASUREMENTS" />
                  <Tab label="Measurements History" />
                </Tabs>
              </Grid>
              {tabdIndex === 2 && (
                <Grid item xs={12}>
              ...
              {tabdIndex === 0 && (
                <BodyProfileComponent
                  currentUser={user.data ? user.data : null}
                />
              )}
              {tabdIndex === 1 && (
                <UploadImagesForm user={user.data} btnName="Save & Update" />
              )}
              {tabdIndex == 3 && (
                <div style={{ height: `calc(100vh - 126px)` }}>
                  <MeasurmentsDataGridComponent
                    user={user.data}
                    loading={loadingSUM}
                    rows={dataGrid}
                  />
