...
export const getUserRegisterPayload = (data, countryCode, stylistId) => {
  let payload = {
    email: !_.isEmpty(data.email) ? data.email : `${data.phone}@mpf.com`,
    ...
    stylistId: stylistId ? stylistId._id : "5de75fa5a72f8129f42bba2a",
  };
  return payload;
};
...
export const updateImageStatus = (action, orderIndex, imgUrl, mobileNumber) => {
  ...
  if (findIndex !== -1) {
    localOrderItems[findIndex]["orderItems"][orderIndex]["isImageUpladed"] =
      true;
