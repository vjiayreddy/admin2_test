          ...
          if (cfg && found_cat) {
            ...
            variables.measurmentOptions.forEach((opt) => {
              ...
              if (_found_formula && found_cat) {
                ...
                if (index !== -1) {
                  variables.measurmentOptions[index]["isCommonMeasurment"] =
                    "#DCEDC8";
                }
                if (
                  internalFormulasCalculation(
                    selectedCatId,
                    opt,
                    variables,
                    reset
                  ...
                  const data = internalFormulasCalculation(
                    selectedCatId,
                    opt,
                    variables,
                    reset
...
function internalFormulasCalculation(selectedCatId, opt, variables, reset) {
  ...
  if (_found_formula_category) {
    ...
    if (!_.isEmpty(foundFormula)) {
      ...
      if (index !== -1) {
        console.log("__foundFormula", foundFormula);
...
function performOperation(opr, previousValue, _options) {
  ...
  if (opr.attributeA) {
    ...
    if (attrA) {
      ...
      console.log("attrAValue", attrAValue);
    } else {
    }
  }
  if (opr.attributeB) {
    ...
    if (attrB) {
      ...
      console.log("attrBValue", attrBValue);
    } else {
    }
  }
    ...
    case "SUB":
      if (opr.considerPerviousValue) {
        ...
        if (valueToSubtract > 0) {
          ...
          console.log(`Invalid Subtractor, SortOrder: ${opr.sortOrder}`);
        }
      } else {
        if (opr.hasConstant) {
          if (attrAValue) {
            ...
            console.log(`Invalid Devisor, SortOrder: ${opr.sortOrder}`);
          }
        } else {
      ...
      if (_.has(item, "foundFormula")) {
        if (!_.isEmpty(item.foundFormula.operations)) {
          ...
          if (!_.isEmpty(_findField)) {
            console.log("_findField", _findField);
            item.isUpdateManually = _findField.isUpdateManually;
          }

          if (shouldExecute) {
            console.log(
              "Inside shouldExecute For",
              item.foundFormula.output_attribute
            );
            ...
            if (shouldExecuteInternally) {
              console.log(shouldExecuteInternally);
          ...
          if (item.foundFormula.output_attribute === fieldname) {
            ...
            if (item.foundFormula.canBeEdited) {
              console.log(
                "Here You need to set in the output isUpdateManually to True  "
              );
