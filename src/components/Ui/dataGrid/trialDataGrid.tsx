...
import TrialTableView from "../tables/trialView";
  ...
  const columns: GridColDef[] = [
        ...
        return (
          <React.Fragment>
            <Button
              ...
              onClick={(e) => {
                if (data.row.serverData.orderTrial) {
                  setTrialData(data.row.serverData);
                  setTrialView(true);
                } else {
                  router.push({
                    pathname: `/trial/form`,
              ...
              size="small"
            >
              {data.row.serverData.orderTrial ? "View" : "Enter"}
            </Button>
          </React.Fragment>
        );
      },
    },
    {
      ...
      renderCell: (data) => {
        return (
          <>
            {data.row.serverData.orderTrial && (
              <select
                style={{ height: 40 }}
                onChange={async (e) => {
                  if (e.target.value !== "NA") {
                    await updateOrderTrialStatus({
                      variables: {
                        orderTrialId: data.row.serverData.orderTrial._id,
              ...
              >
                {initialConfig.status.map((item, index) => (
                  <option
                    selected={item.value === data.row.status}
                    key={index}
                    value={item.value}
                  >
                    {item.name}
          ...
          <>
            {data.row.serverData.orderTrial && (
              <Tooltip
                ...
                title={data.row.serverData.orderTrial?.note}
              >
                <p>{data.row.serverData.orderTrial?.note}</p>
              </Tooltip>
            )}
          </>
        );
