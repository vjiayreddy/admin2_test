...
import { SAVE_LEAD_APPOINTMENT } from "../../../apollo/queries/appointment";
...
const AppointmentForm = ({ onAutoClose, onFormData }) => {
  ...
  const onSubmit = async (data) => {
    const payload = {
      ...
      userId: onFormData.userId,
      leadId: onFormData._leadId,
      appointmentDate: extractAppointDateFormate(
        data.appointmentDate,
        data.appointmentTime
      ),
      appointmentSelectedTimestamp: data.appointmentTime,
  ...
  useEffect(() => {
    if (!_.isEmpty(onFormData)) {
      const payload = {
        appointmentDate: onFormData._appointmentDate,
        stylistIds: onFormData._stylistIds,
        appointmentTime: onFormData._appointmentTime,
        appointmentType: onFormData._appointmentType,
      };
      setStylistId(onFormData._stylistIds);
      setAppointmentID(onFormData.appointmentId);
      reset(payload);
    }
  }, [onFormData]);
