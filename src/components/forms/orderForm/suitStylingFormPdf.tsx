...
const SuitStyleFormPdf: React.FC<styleFormProps> = ({
  data,
  onClose,
  catId,
  orderData,
  ...
  const [trouser, setTrouserItems] = useState([]);
  const [blazer, setBlazerItems] = useState([]);
  ...
  useEffect(() => {
    if (orderItems) {
      ...
      let trouser = [];
      orderItems.map((item) => {
        if (item.label.includes("Blazer")) {
          blazer.push(item);
        } else if (item.label.includes("Trouser")) {
          trouser.push(item);
        }
      });
...
export default SuitStyleFormPdf;
