...
import useStyles from "../measurments/styles";
import {
  CAT_IDS,
  ROLE,
  dropDownOptions,
...
const MeasurementsForm = (props) => {
  ...
  const [SaveUserStandardSizing, { loading: loadingSUSS }] = useMutation(
    SAVE_SIZING_CHART,
    {
      onCompleted({ saveUserStandardSizing }) {
        setIsSubmitted(true);
      },
      refetchQueries: [
        {
          query: GET_USER_STANDARD_SIZING,
          variables: {
            userId: authToken,
  ...
  const [GetStandardSizeChart, { data: dataSizing, loading: loadingSizing }] =
    useLazyQuery(GET_STANDARD_SIZE_CHART);
  const [
    GetUserStandardSizing,
    { data: dataUserSizing, loading: loadingUserSizing },
  ] = useLazyQuery(GET_USER_STANDARD_SIZING, { fetchPolicy: "network-only" });
  ...
  const [saveUserMeasurmentsInfo, { loading: loadingUM }] = useMutation(
    SAVE_USER_MEASURMENTS,
    {
      ...
      onCompleted() {
        alert("Successfully updated user measurements");
  ...
  useEffect(() => {
    if (measurementsAttributes) {
      if (dataSizing?.getStandardSizeChart?.[0].options) {
        let finalValues = [];
        const findOptions = _.find(
          dataSizing?.getStandardSizeChart,
          (item) =>
            item.size === dataUserSizing?.getUserStandardSizing?.[0].size
        );

        if (findOptions?.options?.length > 0) {
          let modifiedOptions =
            dataUserSizing?.getUserStandardSizing?.[0]?.modifiedOptions || [];
          findOptions?.options.map((opt) => {
            const find_Item = _.find(
              modifiedOptions,
              (fi) => fi.name === opt.name
            );
            if (find_Item) {
              if (find_Item.value !== opt.value) {
                finalValues.push(find_Item);
              } else {
                finalValues.push(opt);
              }
            } else {
              finalValues.push(opt);
            }
        ...
        reset(mapUserMeasurements(finalValues));
      }
      getUserMeasurments(router.query?.catId, authToken, subCatId);
    }
  }, [measurementsAttributes, dataSizing]);
  ...
  const onSubmit = async (formData) => {
    ...
    const payload = getSizingPayload(
      measurementsAttributes,
      formData,
      authToken,
      dataUserSizing
  ...
  const savedMeasurments = async () => {
    ...
    payload["standardSizingId"] =
      dataUserSizing?.getUserStandardSizing?.[0]._id;
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={4} md={2}>
              ...
              <input
                disabled
                style={{ width: "100%", height: "35px" }}
                value={dataUserSizing?.getUserStandardSizing?.[0].size}
              />
            </Grid>
          </Grid>
        </Grid>
